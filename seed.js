...
async function main() {
  ...
  console.log('✅  Admin created: admin@vitaminc.in');
  ...
  console.log('✅  Creator created: editor@vitaminc.in');
  ...
  // Load JSON
  const serumsRaw = JSON.parse(fs.readFileSync('src/data/serums.json', 'utf-8'));
  const ingsRaw = JSON.parse(fs.readFileSync('src/data/ingredients.json', 'utf-8'));

  // Brands
  const brandNames = [...new Set(serumsRaw.map(s => s.brand))];
  const brandMap = {};
  ...
  for (const s of serumsRaw) {
    ...
    const scores = {
      ...
      efficacy: s.scores?.ingredient ?? 0,
      stability: s.scores?.stability ?? 0,
      formulation: s.scores?.packaging ?? 0,
